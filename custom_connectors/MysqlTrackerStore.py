...
class MyTrackerStore(TrackerStore):
    ...
    Base = declarative_base()
    #TextBase = declarative_base()
    
    class Conversation(Base):
        __tablename__ = "chat_history"
        id = sa.Column(sa.Integer,_create_sequence(__tablename__),primary_key=True,autoincrement=True)
        chatbotUser = sa.Column(sa.String(255), nullable=False, index=True) #chatbot user id
        utteredBy = sa.Column(sa.String(255), nullable=False)
        message = sa.Column(sa.Text, nullable=False)
        additionalFields = sa.Column(sa.Text)
        createdDate = sa.Column(sa.DateTime, default=sa.func.now())
        updatedDate = sa.Column(sa.DateTime, default=sa.func.now())
        status = sa.Column(sa.Integer, default=1)
        mysql_charset='utf8mb4'

    class UserProvidedDetails(Base):
        __tablename__ = "users"
        userId = sa.Column(sa.Integer,_create_sequence(__tablename__),primary_key=True,autoincrement=True)
        userFirstName = sa.Column(sa.String(100), nullable=False)
        userLastName = sa.Column(sa.String(100), nullable=False)
        NPI = sa.Column(sa.String(100), nullable=False)
        claimId = sa.Column(sa.String(50), nullable=False)
        createdDate = sa.Column(sa.DateTime, default=sa.func.now())
        updatedDate = sa.Column(sa.DateTime, default=sa.func.now())
        status = sa.Column(sa.Integer, default=1)
        mysql_charset='utf8mb4'
    ...
    def __init__(
        ...
        while True:
            try:
                ...
                '''try:
                    self.TextBase.metadata.create_all(self.engine)
                except (
                    sqlalchemy.exc.OperationalError,
                    sqlalchemy.exc.ProgrammingError,
                ) as e:
    ...
    def keys(self) -> Iterable[Text]:
        ...
        with self.session_scope() as session:
            sender_ids = session.query(self.SQLEvent.chatbotUser).distinct().all()
    ...
    def _event_query(self, session: "Session", sender_id: Text) -> "Query":
        ...
        session_start_sub_query = (
            ...
            .filter(
                self.SQLEvent.chatbotUser == sender_id,
        ...
        with self.session_scope() as session:
            ...
            for event in events:
                ...
                if event.type_name == "bot":
                    text = data['text']
                    additional_data = json.dumps(data['data'])
                    #message = json.dumps({'text':text,'data':additional_data})

                    session.add(
                        self.Conversation(
                            chatbotUser=tracker.sender_id,
                            utteredBy=event.type_name,
                            message = text,
